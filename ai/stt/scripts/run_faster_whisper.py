"""Run Faster-Whisper over metadata.csv.
...
DEFAULT_OUTPUT = "ai/stt/results/faster_whisper_result.csv"

FIELDNAMES = ["file_name", "audio_path", "ground_truth", "faster_whisper_text", "faster_whisper_time_sec"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Faster-Whisper transcription for STT metadata rows.")
...
def main() -> None:
    ...
    if runnable_rows:
        model = load_faster_whisper_model(args.model, args.device, args.compute_type)
            ...
            results.append(
                {
                    ...
                    "faster_whisper_text": text,
                    "faster_whisper_time_sec": f"{elapsed:.3f}",
                }
            )
