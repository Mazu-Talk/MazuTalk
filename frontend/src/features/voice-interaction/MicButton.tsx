...
export function MicButton({ disabled, onResult, onListeningChange }: MicButtonProps) {
  ...
  const supported = USE_MOCK ? speech.supported : recorderSupported
  ...
  useEffect(() => {
    if (USE_MOCK && !speech.listening && speech.transcript && !submittedRef.current) {
      submittedRef.current = true
      onResult({ kind: 'text', text: speech.transcript.trim() })
      speech.reset()
    }
  }, [onResult, speech.listening, speech.reset, speech.transcript])
  ...
  if (!supported || typing) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center gap-3">
        <form className="flex w-full gap-2" onSubmit={(event) => { event.preventDefault(); submitDraft() }}>
          <input autoFocus value={draft} disabled={disabled}
            onChange={(event) => setDraft(event.target.value)} placeholder="여기에 말을 적어줘"
          ...
          <button type="submit" disabled={disabled || !draft.trim()}
            className="rounded-2xl bg-brand-500 px-6 py-4 text-xl font-bold text-white shadow-soft active:scale-95 disabled:opacity-40">말하기</button>
        </form>
        {supported && <button onClick={() => setTyping(false)} className="text-base text-brand-500 underline underline-offset-4">🎤 목소리로 말할래요</button>}
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <button onClick={handleMicClick} disabled={disabled} aria-pressed={listening}
        ...
        className={cn(
          ...
          disabled ? 'cursor-not-allowed bg-brand-100 text-brand-300' : listening ? 'bg-emotion-frustrated text-white' : 'bg-brand-500 text-white hover:bg-brand-600',
        )}>
        {listening && !disabled && <span className="absolute inset-0 animate-pulse-ring rounded-full bg-emotion-frustrated/50" />}
        <span aria-hidden>{listening ? '⏹️' : '🎤'}</span>
      </button>
      <p className="h-7 text-lg text-brand-600">{disabled ? '잠깐 기다려줘' : listening ? (USE_MOCK ? speech.interim : '') || '듣고 있어...' : '버튼을 누르고 말해봐'}</p>
      {!listening && !disabled && <button onClick={() => setTyping(true)} className="text-base text-brand-500 underline underline-offset-4">⌨️ 글로 쓸래요</button>}
    </div>
  )
}
