...
from app.services.session_store import create_session, end_session, read_session, read_turns
...
def start_session(payload: SessionCreateRequest):
    return create_session(scenario_id=payload.scenario_id, child_id=payload.child_id)
    ...
    try:
        while True:
            ...
            if event.session_id != session_id:
                await websocket.send_json({
                    "type": "error",
                    "payload": {"message": "세션 ID가 일치하지 않습니다.", "recoverable": True},
                })
                continue
